...
def find_E4(E2, genomic_sequence):  # Finds E4
    ...
    E4_nt = str(E2[(E4_start * 3) + 1:((
                                                   E4_end + 1) * 3) + 1])  # Getting
    # the E4 nucleotide sequence
    E4_nt_start = re.search(E4_nt, str(
        genomic_sequence)).start()  # Finding nuceotide start position of E4
...
def find_E1BS(genome, URR, URRstart, ID, out_dir):
    ...
    for column in csv.reader(open("E1BS/fimo.txt", "rU"),
...
def export_to_mysql(dict, E1BSlong,
                    URRlong):  # Exports start and stop positions for each protein etc
    # to a MySQL database
    connection = pymysql.connect(host='localhost', user='root', password='HobbesSadie96',
                                 db='Practice')
    ...
    with connection.cursor() as cursor:  # With genome
        ...
        if URRlong == 'No' and E1BSlong == 'No':  # Easy case (URR and E1BS don't wrap around)
            cursor.execute(sql, (
            str(dict['accession'][0]), str(dict['name'][0]), str(dict['genome']),
            (str(dict['URR'][0]) + ".." + str(dict['URR'][1])),
            (str(dict['L1'][0]) + ".." + str(dict['L1'][1])),
            (str(dict['L2'][0]) + ".." + str(dict['L2'][1])),
            (str(dict['E1'][0]) + ".." + str(dict['E1'][1])),
            (str(dict['E2'][0]) + ".." + str(dict['E2'][1])),
            (str(dict['E4'][0]) + ".." + str(dict['E4'][1])),
            (str(dict['E5'][0]) + ".." + str(dict['E5'][1])),
            (str(dict['E6'][0]) + ".." + str(dict['E6'][1])),
            (str(dict['E7'][0]) + ".." + str(dict['E7'][1])),
            (str(dict['E1BS'][0]) + ".." + str(dict['E1BS'][1])), str(dict['E2BS'])))

        elif URRlong == 'Yes' and E1BSlong == 'Yes':  # Case where both URR and E1BS wrap around
            cursor.execute(sql, (
            str(dict['accession'][0]), str(dict['name'][0]), str(dict['genome']), (
                        str(dict['URR'][0]) + ".." + str(dict['URR'][1]) + "," + str(
                    dict['URR'][2]) + ".." + str(dict['URR'][3])),
            (str(dict['L1'][0]) + ".." + str(dict['L1'][1])),
            (str(dict['L2'][0]) + ".." + str(dict['L2'][1])),
            (str(dict['E1'][0]) + ".." + str(dict['E1'][1])),
            (str(dict['E2'][0]) + ".." + str(dict['E2'][1])),
            ...
            (str(dict['E6'][0]) + ".." + str(dict['E6'][1])),
            (str(dict['E7'][0]) + ".." + str(dict['E7'][1])), (
                        str(dict['E1BS'][0]) + ".." + str(dict['E1BS'][1]) + "," + str(
                    dict['E1BS'][2]) + ".." + str(dict['E1BS'][3])), str(dict['E2BS'])))
...
def export_from_mysql(choice,
                      accession):  # Exports start and stop positions for each protein
    # etc from a MySQL database
    ...
    with connection.cursor() as cursor:
        if choice == 'name':
            ...
            cursor.execute(sql, accession)
            positions = str(
                str(str(str(cursor.fetchall()).split("'")[1]).split(",")).split("[")[
                    1]).split("]")[0]
            one = int(str(positions.split("'")[1]).split("..")[0])  # Finds position one
            two = int(str(positions.split("'")[1]).split("..")[1])  # Finds position two
            URRlist.append(one)
            URRlist.append(two)
            try:  # Checking to see if the sequence wraps around the genome,
                # if it does, returns all four positions
            ...
            positions = str(cursor.fetchall())
            one = int(
                str(str(str(positions.split("'")[1]).split(",")).split("'")[1]).split(
                    "..")[0])  # Finds position 1 and two
            two = int(
                str(str(str(positions.split("'")[1]).split(",")).split("'")[1]).split(
                    "..")[1])
            E1BSlist.append(one)
            ...
            try:  # Checking to see if the sequence wraps around the genome, if it does, returns all four values
                three = int(
                    str(str(str(positions.split("'")[1]).split(",")).split("'")[3]).split(
                        "..")[0])
                four = int(
                    str(str(str(positions.split("'")[1]).split(",")).split("'")[3]).split(
                        "..")[1])
...
def get_genome_info(
        acccesion):  # Gets all genome information from MySQL database given an
    # accession number and puts all information into a dictionary
    sequences = {}
    options = ['name', 'genome', 'URR', 'L1', 'L2', 'E1', 'E2', 'E4', 'E5', 'E6', 'E7',
        'E10', 'E1BS', 'E2BS']  # NEED GENOME FOR AFTER TESTING
    sequences['accession'] = acccesion
    for values in options:
        sequences.update(export_from_mysql(values, acccesion))

    sequences['L1'] = [sequences['L1'][0], sequences['L1'][1],
        str(sequences['genome'][sequences['L1'][0] - 1:sequences['L1'][1]]), Seq(str(
            sequences['genome'][sequences['L1'][0] - 1:sequences['L1'][1]])).translate(1)]
    sequences['L2'] = [sequences['L2'][0], sequences['L2'][1],
        str(sequences['genome'][sequences['L2'][0] - 1:sequences['L2'][1]]), Seq(str(
            sequences['genome'][sequences['L2'][0] - 1:sequences['L2'][1]])).translate(1)]
    sequences['E1'] = [sequences['E1'][0], sequences['E1'][1],
        str(sequences['genome'][sequences['E1'][0] - 1:sequences['E1'][1]]), Seq(str(
            sequences['genome'][sequences['E1'][0] - 1:sequences['E1'][1]])).translate(1)]
    sequences['E2'] = [sequences['E2'][0], sequences['E2'][1],
        str(sequences['genome'][sequences['E2'][0] - 1:sequences['E2'][1]]), Seq(str(
            sequences['genome'][sequences['E2'][0] - 1:sequences['E2'][1]])).translate(1)]
    sequences['E4'] = [sequences['E4'][0], sequences['E4'][1],
        str(sequences['genome'][sequences['E4'][0] - 1:sequences['E4'][1]]), Seq(str(
            sequences['genome'][sequences['E4'][0] - 1:sequences['E4'][1]])).translate(1)]
    sequences['E5'] = [sequences['E5'][0], sequences['E5'][1],
        str(sequences['genome'][sequences['E5'][0] - 1:sequences['E5'][1]]), Seq(str(
            sequences['genome'][sequences['E5'][0] - 1:sequences['E5'][1]])).translate(1)]
    sequences['E6'] = [sequences['E6'][0], sequences['E6'][1],
        str(sequences['genome'][sequences['E6'][0] - 1:sequences['E6'][1]]), Seq(str(
            sequences['genome'][sequences['E6'][0] - 1:sequences['E6'][1]])).translate(1)]
    sequences['E7'] = [sequences['E7'][0], sequences['E7'][1],
        str(sequences['genome'][sequences['E7'][0] - 1:sequences['E7'][1]]), Seq(str(
            sequences['genome'][sequences['E7'][0] - 1:sequences['E7'][1]])).translate(1)]

    if sequences[
        'E10'] == 'Null':  # Seeing if E10 was found, if not it it deleted from sequence dictionary
        del sequences['E10']
    else:  # If E10 exists then sequence and translated sequence are found
        sequences['E10'] = [sequences['E10'][0], sequences['E10'][1],
            str(sequences['genome'][sequences['E10'][0] - 1:sequences['E10'][1]]), Seq(
                str(sequences['genome'][
                    sequences['E10'][0] - 1:sequences['E10'][1]])).translate(1)]
    try:  # Case where URR wraps around,Need to look at the sequence slicing
        sequences['URR'] = [sequences['URR'][0], sequences['URR'][1], sequences['URR'][2],
            sequences['URR'][3], str(
                sequences['genome'][sequences['URR'][0] - 1:] + sequences['genome'][
                sequences['URR'][2] - 1:sequences['URR'][3] - 1])]
    except IndexError:  # Case where URR does not wrap around, Need to look at the sequence slicing
        sequences['URR'] = [sequences['URR'][0], sequences['URR'][1],
            str(sequences['genome'][sequences['URR'][0] - 1:sequences['URR'][1] - 1])]
    try:  # Case where E1BS wraps around,Need to look at the sequence slicing
        sequences['E1BS'] = [sequences['E1BS'][0], sequences['E1BS'][1],
            sequences['E1BS'][2], sequences['E1BS'][3], str(
                sequences['genome'][sequences['E1BS'][0] - 2:] + sequences['genome'][
                sequences['E1BS'][2] - 1:sequences['E1BS'][3] - 1])]
    except IndexError:  # Case where E1BS does not wrap around,Need to look at the sequence slicing
        sequences['E1BS'] = [sequences['E1BS'][0], sequences['E1BS'][1],
            str(sequences['genome'][sequences['E1BS'][0] - 1:sequences['E1BS'][1] - 1])]
    ...
    for value in annotations:
        if value == 'genome':
            ...
            try:
                out_file.writerows([[annotations['accession'], value,
                    'join(' + str(annotations[value][0]) + '..' + str(
                        ...
                        annotations[value][2]) + ".." + str(annotations[value][3]) + ')',
                    annotations[value][4]]])
            except IndexError:
                out_file.writerows([[annotations['accession'], value,
                    str(annotations[value][0]) + '..' + str(annotations[value][1]),
            ...
            for i in range(0, len(annotations[value]), 1):
                out_file.writerows([[annotations['accession'], value,
                    str(annotations[value][i]) + '..' + str(annotations[value][i] + 11),
                    annotations['genome'][
                    annotations[value][i] - 1:annotations[value][i] + 11]]])
