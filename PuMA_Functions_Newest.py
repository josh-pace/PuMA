...
def Blast(protein_sequence, start, end, genomic_sequence):#This function uses blast to find the different proteins in the translated genome
    ...
    if M:
        ...
        blasting = blastp(query=tempfile, db="/Users/joshpace/Downloads/Blast/DB.blast.txt", evalue=0.001, outfmt=5,out="/Users/joshpace/Downloads/Blasted.xml")
        blasting()
            ...
            if DEF.group(1) == 'L1':
                ...
                if spliced:
                    ...
                    if start_L1 % 3 == 0:
                        if start_L1 > 600:
                            L1_post = L1_pre
                            result['L1'] = [start_L1, end]  #, str(L1_post), Seq(str(L1_post)).translate()]
                        else:
                            L1_post = L1_pre[start_L1:]
                            result['L1'] = [int(real_start + 1) + int(start_L1), str(end)] #, str(L1_post), Seq(str(L1_post)).translate()]
                    else:
                        L1_post = L1_pre
                        result['L1'] = [real_start + 1, end] #, L1_post, Seq(str(L1_post)).translate()]
                else:
                    L1_post = L1_pre
                    result['L1'] = [real_start + 1, end] #, L1_post, Seq(str(L1_post)).translate()]
            else:
                real_start = start + M.start() + M.start() + M.start()
                result[DEF.group(1)] = [real_start + 1, end] #, genomic_sequence[int(real_start):int(end)], query]
...
def find_E2BS(genome,URR, URRstart,ID):#This function finds the E2BS in a genome using the URR
    ...
    startListURR = map(int, set(startListURR))#Making the positions an integer value to use as index and only using every unique start position
    startListGenome = map(int, startListGenome)#Making the positions an integer value
...
def find_E1BS(genome,URR,URRstart,ID):
    ...
    for column in csv.reader(open("/Users/joshpace/Google Drive/Independent Study (BME 492(F2017:SP2018))/PycharmProjects/PuMA/E1BS/fimo.txt", "rU"), delimiter='\t'):  # Getting nucleotide start positions from FIMO output file
        if column[3] == 'start':
            startListURR = []
        else:
            startListURR.append(column[3])
