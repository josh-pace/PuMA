...
def find_E2BS(genome,URR, URRstart,ID):#This function finds the E2BS in a genome using the URR
    ...
    cline = ("fimo --norc --verbosity 1 --thresh 1.0E-3 meme_3000_TOTAL.txt PuMA_URR_tempfile.fa")#Executing FIMO, Using URR
    os.system(str(cline))  # Executing FIMO

    for column in csv.reader(open("fimo_out/fimo.txt", "rU"), delimiter='\t'):  # Getting nucleotide start positions from FIMO output file
        if column[3] == 'start':
            startListURR = []
        else:
...
def find_E1BS(genome,URR,URRstart,ID):
    ...
    with open("PuMA_URR_tempfile.fa", "w") as tempfile:#Writting URR to a file so FIMO can be used
        ...
        # print >> tempfile, '>URR for %s' %ID
        # print >> tempfile, URR
    cline = ("fimo  --norc --verbosity 1 --thresh 1.0E-4 --bgfile background_model_E1BS.txt meme_E1BS_1motif_18_21.txt PuMA_URR_tempfile.fa")#Executing FIMO, Using URR
    os.system(str(cline))  # Executing FIMO
